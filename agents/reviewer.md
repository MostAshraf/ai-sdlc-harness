...
description: >
  ...
  pre-pr | analyze-comments | request-triage). Never invoke directly.
...
`review-task.md` · `plan-review` → `plan-review-task.md` · `pre-pr` →
...
- `plan-review`      → pre-gate review of `<run>/plan.md` (no task header):
  AC coverage, conventions consistency, dependency/contract audit, scope
  containment. Verdict APPROVED or CHANGES_REQUESTED — it mechanically
  drives the plan revision loop.
